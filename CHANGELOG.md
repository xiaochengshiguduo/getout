...
- 更新流程不再退出管理面板：原先 `update_getout` 用 `exec getout restart` 替换当前进程导致面板消失，改为普通调用并 `return 0`，重启后回到主菜单继续操作。

## 2.0.9

- 管理面板改为循环交互：每个操作完成后停留并按回车返回主菜单，不再做完一个操作就退出，支持连续操作。
- 单个菜单操作用子 shell 隔离，操作内部 `fatal`/异常只回到主菜单，不会踢出整个面板。
- 卸载成功后直接退出面板（与旧行为一致）；取消卸载则返回主菜单。
...
- 抽取状态页地址打印逻辑，减少入口/出口状态输出重复代码。

## 2.0.5

- 抽取 SOCKS5/WireGuard 出口配置的公共运行字段写入逻辑，减少重复。
...
- 合并入口/出口回滚快照与恢复逻辑，不改变运行行为。

## 2.0.2

- 修改入口信息子菜单统一使用数字选项 `1/2`，与管理面板主菜单保持一致。
