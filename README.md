...
- `关闭 SOCKS5 模式` / `启动 SOCKS5 模式`
- `关闭 WireGuard 模式` / `启动 WireGuard 模式`
- `关闭 s5-V4 单栈模式` / `启动 s5-V4 单栈模式`
- `关闭 s5-V4+V6 双栈模式` / `启动 s5-V4+V6 双栈模式`
- `关闭 WG-V4 单栈模式` / `启动 WG-V4 单栈模式`
- `关闭 WG-V4+V6 双栈模式` / `启动 WG-V4+V6 双栈模式`
- `切换至 V4 优先模式` / `切换至 V6 优先模式`
...
密钥处理：
- 服务端私钥留空自动生成，同时显示派生公钥（需复制到客户端配置）
- 对端公钥留空自动生成客户端密钥对，显示客户端私钥（需复制到客户端配置）
